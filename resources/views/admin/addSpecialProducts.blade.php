...
@section('content')
        ...
        <div class="content-page">
            ...
            <div class="page-heading">
                <h1><i class='fa fa-bell-o'></i> Special Products</h1>
<!--                <h3>Create Product Categories</h3>-->
            </div>
                
            <div class="widget">
							<div class="widget-header transparent">
								<h2><strong>Add</strong> Special Products</h2>
                                ...
                                <div class="table-responsive">
									<table data-sortable class="table table-hover table-striped">
                                        ...
                                        <!-- ----------------------------------------------- -->
                                                ...
                                                @if(!count($special))
                                                <td><a href="add-this-product/{{ $cat->productId}}" class="btn btn-blue-3">Add</a></td>
                                                <td><a href="delete-this-specialProduct/{{ $cat->productId}}" class="btn btn-orange-1" disabled>Remove</a></td>
                                                @endif
                                        ...
                                        @if(count($special))
                                            @foreach($special as $key=>$sp)
                                                ...
                                                @if($cat->productId == $sp->productId)
                                                  <td><a href="add-this-product/{{ $cat->productId}}" class="btn btn-blue-3" {{ $cat->productId == $sp->productId ? 'disabled' : ''}}> {{ $cat->productId == $sp->productId ? 'Added' : 'Add'}}</a></td>
                                                
                                                  <td><a href="delete-this-specialProduct/{{ $cat->productId}}" class="btn btn-orange-1" {{ $cat->productId == $sp->productId ? '' : 'disabled'}}>Remove</a></td>
                                                <?php $flag=1; ?>
                                                @endif
